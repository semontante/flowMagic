...
magicTrain_local<-function(list_train_sets,n_tree=10,train_model="rf",n_cores=1){
  set.seed(40)
  ...
  for(level_name in all_levels_name){
    ...
    for(train_set_pop in level_pops){
      ...
      ref_model_info<-magicTrain_plain_one_model(df_train = train_set_pop,n_cores = n_cores,train_model = train_model,
                                                      list_index = NULL,n_tree = n_tree)
...
#' @param list_index  List of vector of indices to use in training generated by the get_indices_cross_val.
...
magicTrain<-function(df_train,n_cores=1,train_model="rf",k_cv=10,
                                     list_index=NULL,n_tree=10,tune_lenght=3,size_nnet_units=100,
                                     decay_nnet=0.1,method_control="oob"){
  start<-Sys.time()
  ...
  Ytrain<-as.character(df_info[,"classes"])
  # train model
  print("training...")
  ...
  if(train_model=="rf"){
    out_model<-magicTrain_rf(Xtrain = Xtrain,Ytrain = Ytrain,
                             list_index = list_index,n_tree = n_tree,
    ...
    out_model<-magicTrain_dt(Xtrain = Xtrain,Ytrain = Ytrain,k_cv = k_cv,
                             list_index = list_index,tune_lenght = tune_lenght)
    
  }else if(train_model=="nnet"){
    out_model<-magicTrain_nnet(Xtrain = Xtrain,Ytrain = Ytrain,k_cv = k_cv,
                               list_index=list_index,size = size_nnet_units,
                               decay = decay_nnet)
  }
  stopCluster(cl)
  end<-Sys.time()
...
#' @param k_cv  Number of k for cross-validation (if method control=cv)
#' @param list_index  List of vector of indices to use in training.
#' @param tune_lenght Number of parameters values trained during cross-validation.
#' @return model object.
#' @export
#' @examples 
...
magicTrain_dt<-function(Xtrain,Ytrain,k_cv=10,list_index=NULL,tune_lenght=10){
...
magicTrain_nb<-function(Xtrain,Ytrain,k_cv=10){
...
magicTrain_nnet<-function(Xtrain,Ytrain,k_cv=10,list_index=NULL,size=100,decay=0.1){
...
magicTrain_rf<-function(Xtrain,Ytrain,list_index=NULL,n_tree=10,method_control="oob",k_cv=10){
