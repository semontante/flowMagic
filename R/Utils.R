...
get_indices_cross_val<-function(df_train,n_cores=1){
  all_plot_num<-unique(df_train$plot_num)
  list_inds_cross_val<-mclapply(1:length(all_plot_num),function(i){
    current_plot_num<-all_plot_num[i]
    inds<-which(df_train$plot_num==current_plot_num)
    return(inds)
  },mc.cores = n_cores)
  return(list_inds_cross_val)
}
...
#' @export
#' @examples 
#' \donttest{get_paths_training()}

get_paths_training<-function(df_paths,n_paths=200,seed_n=40){
  set.seed(seed_n)
  df_paths$path<-as.character(df_paths$path)
  df_paths$p_num<-as.integer(df_paths$p_num)
  all_gates<-unique(df_paths$p_num)
  paths_selected<-c()
  print(all_gates)
  for(g in all_gates){
    path_data_to_train_n_gates_g<-df_paths[which(df_paths$p_num==g),1]
    if(length(path_data_to_train_n_gates_g)<n_paths){
      path_data_to_train_n_gates_g_selected<-path_data_to_train_n_gates_g
      paths_selected<-c(paths_selected,path_data_to_train_n_gates_g_selected)

    }else{
      inds_paths<-sample.int(n = length(path_data_to_train_n_gates_g),size = n_paths)
      path_data_to_train_n_gates_g_selected<-path_data_to_train_n_gates_g[inds_paths]

      paths_selected<-c(paths_selected,path_data_to_train_n_gates_g_selected)
    }
  }
  paths_selected<-str_remove(paths_selected,"/home/rstudio/mnt/FCS_local5/projects/")
  paths_selected<-str_replace(paths_selected,"Discovery","/home/rstudio/Discovery_2")
...
get_paths_training_v2<-function(df_paths,n_paths=200,seed_n=40){
  ...
  for(g in all_gates){
    inds_paths_g<-which(df_paths[,3]==g)
    path_data_to_train_n_gates_g_data<-df_paths[inds_paths_g,1]
    path_data_to_train_n_gates_g_classes<-df_paths[inds_paths_g,2]
    if(length(path_data_to_train_n_gates_g_data)<n_paths){
      path_data_to_train_n_gates_g_selected_data<-path_data_to_train_n_gates_g_data
      path_data_to_train_n_gates_g_selected_classes<-path_data_to_train_n_gates_g_classes
    }else{
      inds_paths<-sample.int(n = length(path_data_to_train_n_gates_g_data),size = n_paths)
      path_data_to_train_n_gates_g_selected_data<-path_data_to_train_n_gates_g_data[inds_paths]
      path_data_to_train_n_gates_g_selected_classes<-path_data_to_train_n_gates_g_classes[inds_paths]
    }
  ...
  paths_selected_classes<-str_remove(path_selected_classes,"/mnt/FCS_local5/projects/")
  paths_selected_classes<-str_replace(paths_selected_classes,"Discovery","/home/rstudio/Discovery_2")
  
  df_paths_selected<-as.data.frame(cbind(paths_selected_data,paths_selected_classes),stringsAsFactors=F)
  
  return(df_paths_selected)
...
#' @param prop_down Proportion of events (downsampling). Default to 1 (no downsampling).
#' @return Dataframe.
#' @export
#' @examples 
#' \donttest{process_test_data()}

process_test_data<-function(test_data,prop_down=1){
  names_dens_features<-c("n_peaks_m1","h_peak_m1_1","pos_peak_m1_1","start_peak_m1_1","end_peak_m1_1",
                         "h_peak_m1_2","pos_peak_m1_2","start_peak_m1_2","end_peak_m1_2",
                         "h_peak_m1_3","pos_peak_m1_3","start_peak_m1_3","end_peak_m1_3",
                         "h_peak_m1_4","pos_peak_m1_4","start_peak_m1_4","end_peak_m1_4",
